...
import dotvisualizer.transforms.RemoveTempWires
...
object VisualizerAnnotation {
  ...
  def setOutputFormat(format: String): Annotation = {
    VisualizerAnnotation(CircuitName("Visualizer"), s"${Visualizer.OutputFormat}=$format")
  }
  ...
  def setStartModule(startModule: String): Annotation = {
    VisualizerAnnotation(CircuitName("Visualizer"), s"${Visualizer.StartModuleString}=$startModule")
  }
      ...
      def processStatement(s: Statement): Unit = {
        s match {
          ...
          case WDefInstance(_, instanceName, moduleName, _) =>
            ...
            processModule(newPrefix, subModule, subModuleNode, scope.descend)
  ...
  override def execute(state: CircuitState): CircuitState = {
    ...
    val filteredAnnotations = state.annotations.flatMap {
      case annotation@VisualizerAnnotation(_, value) =>
        ...
        else if(value.startsWith(Visualizer.StartModuleString)) {
          startModule = value.split("=", 2).last.trim
          None
        }
        else {
          Some(annotation)
    ...
    val pass_top_level = new ModuleLevelGrapher(targetDir, backFileName = "TopOfVisualizer")
    pass_top_level.run(state.circuit)

    if(startModule.isEmpty) {
      startModule = circuit.main
    }

    queue += startModule // top level visualizer
    ...
    val fileName = s"$targetDir$startModule.dot"
...
object Visualizer {
  ...
  val StartModuleString = "StartModule"
  ...
  def run(
    ...
    val annotations = AnnotationSeq(
      Seq(
        ...
        (if(startModule.nonEmpty) { Seq(VisualizerAnnotation.setStartModule(startModule)) } else { Seq.empty })
    )
  ...
  def main(args: Array[String]): Unit = {
    args.toList match {
      case fileName :: startModule :: dotProgram :: openProgram :: outputFormat :: Nil =>
        run(fileName, dotProgram, openProgram, outputFormat, startModule = startModule)
      case fileName :: startModule :: dotProgram :: openProgram :: Nil =>
        run(fileName, dotProgram, openProgram, startModule = startModule)
      case fileName :: startModule :: dotProgram :: Nil =>
        run(fileName, dotProgram, startModule = startModule)
      case fileName :: startModule :: Nil =>
        run(fileName, startModule = startModule)
      case fileName :: Nil =>
        run(fileName)
      case _ =>
